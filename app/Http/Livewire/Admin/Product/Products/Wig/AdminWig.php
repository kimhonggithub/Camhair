...
namespace App\Http\Livewire\Admin\Product\Products\Wig;
...
class AdminWig extends AdminComponent
{
    use WithFileUploads;
    ...
    {
        $cat = Category::where('name', 'Wig Hair')->first()->id;
        $this->validate();
    ...
    public function render()
    {
        ...
        $cat_name = Category::where('name','Wig Hair')->first()->name;
        $products = Product::search($this->searchTerm, $cat)
            ->orderBy($this->sortColumnName, $this->sortDirection)
            ->paginate(5);
        return view('livewire.admin.product.products.wig.admin-wig', [
            'colors' => $colors,
            'Patterns' => $Patterns,
            'products' => $products,
            'lenghts' => $lenghts,
